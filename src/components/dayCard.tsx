  ...
  return (
    <div className="card" onClick={() => openDay()}>
      ...
      <div className="cardData">
        {"Активных задач: " +
          tasks.filter((task: ITask) => task.owner === activeProfile && new Date(task.date).getDate() === day && task.state === false).length}
      </div>
      <div className="cardData">
        {"Выполненных задач: " +
          tasks.filter((task: ITask) => task.owner === activeProfile && new Date(task.date).getDate() === day && task.state === true).length}
      </div>
    </div>
  );
};
